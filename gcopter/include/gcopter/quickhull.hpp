...
class MeshBuilder {
 public:
  ...
  inline std::array<size_t, 2> getVertexIndicesOfHalfEdge(
...
template <typename T>
inline Vector3<T> getTriangleNormal(const Vector3<T> &a, const Vector3<T> &b,
                                    const Vector3<T> &c) {
...
class QuickHull {
  ...
  // QuickHull iteration proceeds. m_extremeValues must be properly set up when
  // this is called.
  inline void setupInitialTetrahedron() {
    ...
    if (vertexCount <= 4) {
      ...
      const Vector3<T> N = mathutils::getTriangleNormal(
    ...
    for (size_t i = 0; i < 6; i++) {
      for (size_t j = i + 1; j < 6; j++) {
        const T d = m_vertexData[m_extremeValues[i]].getSquaredDistanceTo(
    ...
    if (maxD == m_epsilonSquared) {
      ...
      const size_t thirdPoint = (it == m_vertexData.end())
                                    ...
                                    : std::distance(m_vertexData.begin(), it);
      ...
      return m_mesh.setup(selectedPoints.first, selectedPoints.second,
    ...
    if (maxD == m_epsilon) {
      ...
      m_planarPointCloudTemp.insert(m_planarPointCloudTemp.begin(),
                                    m_vertexData.begin(), m_vertexData.end());
  ...
  inline std::array<size_t, 6> getExtremeValues() {
    ...
    T extremeVals[6] = {m_vertexData[0].x, m_vertexData[0].x,
  ...
  inline T getScale(const std::array<size_t, 6> &extremeValues) {
    T s = 0;
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[0]]) + 0)));
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[1]]) + 0)));
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[2]]) + 1)));
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[3]]) + 1)));
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[4]]) + 2)));
    s = std::max(s,
                 std::abs(*((const T *)(&m_vertexData[extremeValues[5]]) + 2)));
  ...
  inline bool addPointToFace(typename MeshBuilder<T>::Face &f,
                             size_t pointIndex);
  ...
  inline void createConvexHalfEdgeMesh() {
    ...
    while (!m_faceList.empty()) {
      ...
      while (m_possiblyVisibleFaces.size()) {
            ...
            for (auto heIndex : m_mesh.getHalfEdgeIndicesOfFace(pvf)) {
              if (m_mesh.m_halfEdges[heIndex].m_opp !=
                ...
                m_possiblyVisibleFaces.emplace_back(
                    m_mesh.m_halfEdges[m_mesh.m_halfEdges[heIndex].m_opp]
                        .m_face,
                    heIndex);
              }
            }
        ...
        // the face will not be part of the final mesh so their data slots can
        // by recycled.
        ...
        m_mesh
            .m_faces[m_mesh.m_halfEdges[faceData.m_enteredFromHalfEdge].m_face]
      ...
      for (auto faceIndex : m_visibleFaces) {
        ...
        if (t) {
          // Because we should not assign point vectors to faces unless
          // needed...
          assert(t->size());
      ...
      for (size_t i = 0; i < horizonEdgeCount; i++) {
        ...
        const Vector3<T> planeNormal = mathutils::getTriangleNormal(
  ...
  inline void buildMesh(const VertexDataSource<T> &pointCloud, bool CCW,
                        bool useOriginalIndices, T eps) {
  ...
  inline ConvexHull<T> getConvexHull(const VertexDataSource<T> &pointCloud,
                                     bool CCW, bool useOriginalIndices, T eps) {
 ...
 public:
  ...
  //   should the output mesh use same vertex indices as the
  //                       original point cloud. If this is false, then we
  //                       generate a new vertex buffer which contains only the
  //                       vertices that are part of the convex hull.
  //   eps: minimum distance to a plane to consider a point being on positive
  //   side
  //        of it (for a point cloud with scale 1)
  inline ConvexHull<T> getConvexHull(const T *vertexData, size_t vertexCount,
                                     bool CCW, bool useOriginalIndices,
                                     T eps = defaultEps<T>()) {
  ...
  //   distance to a plane to consider a point being on positive side
  //        of it (for a point cloud with scale 1)
  // Returns:
...
template <typename T>
inline void QuickHull<T>::reclaimToIndexVectorPool(
    std::unique_ptr<std::vector<size_t>> &ptr) {
...
template <typename T>
inline bool QuickHull<T>::addPointToFace(typename MeshBuilder<T>::Face &f,
                                         size_t pointIndex) {
  const T D =
      mathutils::getSignedDistanceToPlane(m_vertexData[pointIndex], f.m_P);

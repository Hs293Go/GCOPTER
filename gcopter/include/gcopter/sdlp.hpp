...
template <int d>
inline int lp_no_con(const double *n_vec, const double *d_vec, double *opt) {
  double n_dot_d = 0.0;
  double d_dot_d = 0.0;
  for (int i = 0; i <= d; i++) {
...
inline void lp_min_lin_rat(const bool degen, const double cw_vec[2],
                           const double ccw_vec[2], const double n_vec[2],
...
inline int wedge(const double (*halves)[2], const int m, int *next, int *prev,
                 double cw_vec[2], double ccw_vec[2], bool *degen) {
  int i;
  double d_cw, d_ccw;
  bool offensive;
  ...
  *degen = false;
  ...
  while (i != m) {
    ...
    if (d_ccw >= 2.0 * eps) {
      ...
      return INFEASIBLE;
    } else if (d_cw <= -2.0 * eps || d_ccw <= -2.0 * eps ||
...
template <int d>
inline void findimax(const double *pln, int *imax) {
  *imax = 0;
...
*/
{
  ...
  for (int i = 0; i != m; i = next[i]) {
    /* if the optimum is not in half space i then project the problem onto that
     * plane */
    ...
    if (val < -(d + 1) * eps) {
      ...
      /* solve sub problem */
      status = linfracprog<d - 1>(new_halves, max_size, i, new_n_vec, new_d_vec,
...
template <>
inline int linfracprog<1>(const double *halves, const int max_size, const int m,
...
*/
{
  ...
  /* original allocated size is m, here changed to m + 1 for legal tail
   * accessing */

...
class GlobalPlanner {
 ...
 public:
  GlobalPlanner(const Config &conf, ros::NodeHandle &nh_)
    ...
    targetSub =
        nh.subscribe(config.targetTopic, 1, &GlobalPlanner::targetCallBack,
  ...
  inline void plan() {
    if (startGoal.size() == 2) {
      ...
      sfc_gen::planPath<voxel_map::VoxelMap>(
          startGoal[0], startGoal[1], voxelMap.getOrigin(),
      ...
      sfc_gen::convexCover(route, pc, voxelMap.getOrigin(),
                           voxelMap.getCorner(), 7.0, 3.0, hPolys);
      ...
      if (route.size() > 1) {
        ...
        // magnitudeBounds = [v_max, omg_max, theta_max, thrust_min,
        // thrust_max]^T penaltyWeights = [pos_weight, vel_weight, omg_weight,
        // theta_weight, thrust_weight]^T physicalParams = [vehicle_mass,
    ...
    if (mapInitialized) {
      ...
      const double zGoal = config.mapBound[4] + config.dilateRadius +
                           fabs(msg->pose.orientation.z) *
                               (config.mapBound[5] - config.mapBound[4] -
                                2 * config.dilateRadius);
      const Eigen::Vector3d goal(msg->pose.position.x, msg->pose.position.y,
                                 zGoal);
